...
contract SafemoonSupportTemplate is Context, IBEP20, Ownable {
  ...
  // Fee that is separated and used to purchase and burn Safemoon when burnSafemoon() is called
  uint256 private _sfmFee;
  ...
  */
   ...
   */ 
  function safemoonFee() external view returns (uint256) {
    return _sfmFee;
  }
  ...
  /** 
   * Restores Safemoon Purchase Fee to Previous Value
   */  
  function restoreSFMFee() public onlyOwner {
      _sfmFee = _previousSfmFee;
  }
  
  /**
   * The Token Contract Balance Threshhold needed to call burnSafemoon()
   */
  function safemoonBurnThreshhold() external view returns (uint256) {
      return _safemoonBurnThreshhold;
  }
  
  /**
   * The rate at which new tokens are minted each time burnSafemoon() is called.
   */
  ...
  /**
   * Set the rate at which new tokens are minted each time burnSafemoon() is called.
   */ 
  function setMintRate(uint256 newRate) public onlyOwner {
      _mintRate = newRate;
  }
  
  /**
   * Enables or Disables The Mint Property To SafemoonSupport
   * If Enabled mint() is called during burnSafemoon function
   */
   ...
   * Splits apart _sfmFee percent of the transaction and stores in Contract Address
   * If Address reaches safemoonBurnThreshhold, safemoonBurn() will be called
   * _mint() will also be called if _canMintTokens is set to True
   *
   * Emits a {Transfer} event.
   *
   * Requirements:
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    // Remove Fee from transaction
    uint256 fee = _calculateSafemoonFee(amount);
    // Amount recipient will receive
    uint256 transferAmount = amount.sub(fee);
    
    // If the contract's balance is greater than or equal to threshhold, swap for Safemoon and send to burn wallet
    ...
    // Give Reduced Amount to Recipient
    _balances[recipient] = _balances[recipient].add(transferAmount);
    // Store Transaction Fee in this contract
  ...
  /**
  *
  * Returns the fee taken out of every transaction to be kept in the Contract's Address
  *
  */
